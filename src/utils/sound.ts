...
export { calculateCents, calculateFrequency, calculateNote, calculateSemiTone, logError, throwError, toDecimals }
...
export async function loadAudioFromFile (blob: Blob): Promise<AudioBuffer> {
  ...
  return await loadFile.then(
    async (arrayBuffer) => await offlineCtx.decodeAudioData(arrayBuffer))
}
